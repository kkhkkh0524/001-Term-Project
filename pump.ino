/*
이 코드는 개별 동작을 가정하고 쓰인 코드이므로 나중에 main 코드와 결합 시 적절한 수정이 필요하다. 
사용자 입력 인터페이스에서 적절한 값을 얻어서 전역변수로 사용하며, 설정된 값은 임시값이다.

5V 정격 기준 (4.5V 인가) 펌프의 출수량 = 1000ms 당 27.7mL -> 36.1ms 당 1mL 출수 

watering_amount 만큼 급수하기 위해선 펌프는 최소 (watering_amount * 36.1) ms 만큼 작동
펌프 작동 시간 = (watering_amount * 36.1) + (물의 배관 이동시간) 
*/
